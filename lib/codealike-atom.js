...
const AtomCodealikePlugin = {
    ...
    if (Codealike.hasUserToken()) {
        ...
        Codealike.connect()
                .then(
                    ...
                    function() {
                        AtomCodealikePlugin.statusBarView.updateMessage("Codealike is not connected. Please configure your token to start using Codealike.", "warning");
    ...
    .then(
      function(token) {
        ...
        if (token) {
            ...
            AtomCodealikePlugin.statusBarView.updateMessage("Codealike is disconnected", "info");
        }
      },
      function() {
  ...
  activate(state) {
    ...
    Codealike.initialize('atom', '0.0.5');
